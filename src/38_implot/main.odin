...
init_app :: proc(app: ^App) -> (err: ovk.Error) {
	...
	// Create window
	app.window = ovk.create_window({instance = &app.instance, title = "ImPlot", width = 1200, height = 800, resizable = true}) or_return

	// Register the Escape key callback. The order relative to init_imgui does
	// not matter: ovk chains to the GLFW callback already installed by ImGui,
	// and ImGui chains to previously installed callbacks, so both work either
	// way.
...
run_app :: proc(app: ^App) -> (err: ovk.Error) {
	...
	for !ovk.window_should_close(&app.window) && app.running {
		...
		// Start an ImGui frame and build the UI. The ImPlot demo window shows
		// off the plotting API, from simple lines to subplots and drag tools.
		ovk.imgui_new_frame()
