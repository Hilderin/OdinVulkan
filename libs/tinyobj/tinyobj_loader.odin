...
parse_mtl_file :: proc(filename: string) -> ([dynamic]Material, map[string]int, bool) {
	...
	for line in strings.split_lines_iterator(&it) {
		...
		case "Ni":
			if len(parts) >= 2 {
				current_mat.ior = parse_float(parts[1])
			}
		case "Ke":
			if len(parts) >= 4 {
		...
		case "Ns":
			if len(parts) >= 2 {
				current_mat.shininess = parse_float(parts[1])
			}
		case "d":
			if len(parts) >= 2 {
				current_mat.dissolve = parse_float(parts[1])
			}
		case "Tr":
			if len(parts) >= 2 {
				current_mat.dissolve = 1.0 - parse_float(parts[1])
			}
		case "illum":
			if len(parts) >= 2 {
				current_mat.illum = parse_int(parts[1])
			}
		case "map_Ka":
			if len(parts) >= 2 {
				current_mat.ambient_texname = strings.clone(parts[1])
			}
		case "map_Kd":
			if len(parts) >= 2 {
				current_mat.diffuse_texname = strings.clone(parts[1])
			}
		case "map_Ks":
			if len(parts) >= 2 {
				current_mat.specular_texname = strings.clone(parts[1])
			}
		case "map_Ns":
			if len(parts) >= 2 {
				current_mat.specular_highlight_texname = strings.clone(parts[1])
			}
		case "map_bump", "bump":
			if len(parts) >= 2 {
				current_mat.bump_texname = strings.clone(parts[1])
			}
		case "disp":
			if len(parts) >= 2 {
				current_mat.displacement_texname = strings.clone(parts[1])
			}
		case "map_d":
			if len(parts) >= 2 {
				current_mat.alpha_texname = strings.clone(parts[1])
			}
		}
	}
...
destroy :: proc(o: ^OBJ) {
	...
	for m in o.materials {
		delete(m.name)
		if len(m.ambient_texname) > 0 {
			delete(m.ambient_texname)
		}
		if len(m.diffuse_texname) > 0 {
			delete(m.diffuse_texname)
		}
		if len(m.specular_texname) > 0 {
			delete(m.specular_texname)
		}
		if len(m.specular_highlight_texname) > 0 {
			delete(m.specular_highlight_texname)
		}
		if len(m.bump_texname) > 0 {
			delete(m.bump_texname)
		}
		if len(m.displacement_texname) > 0 {
			delete(m.displacement_texname)
		}
		if len(m.alpha_texname) > 0 {
			delete(m.alpha_texname)
		}
	}
	delete(o.materials)
}
