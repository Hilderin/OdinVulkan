...
init_app :: proc(app: ^App) -> (err: ovk.Error) {
	...
	// Create window
	app.window = ovk.create_window({instance = &app.instance, title = "Timeline semaphores", width = 512, height = 512, resizable = true}) or_return
	...
	for !ovk.window_should_close(&app.window) && app.running {
		...
		if counter_value, counter_err := ovk.get_semaphore_counter_value(&app.swap_chain.frame_timeline); counter_err == nil {
			if counter_value % 60 == 0 {
				fmt.printfln("Timeline semaphore counter: %d", counter_value)
			}
		}
	}
