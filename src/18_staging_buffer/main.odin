...
mem_copy_to_buffer :: proc(device: vk.Device, buffer_memory: vk.DeviceMemory, data: []$T) -> bool {
	...
	result := vk.MapMemory(device, buffer_memory, 0, vk.DeviceSize(size), {}, cast(^rawptr)&dest_data)
	if result != .SUCCESS {
...
main :: proc() {
    ...
    // odinfmt: enable
	vertex_buffer: vk.Buffer
	vertex_buffer, ok = create_buffer(device, u64(size_of(Vertex) * len(vertices)), {.VERTEX_BUFFER})
	if !ok {
