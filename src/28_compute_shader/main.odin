...
submit_command_buffer :: proc(
	...
	submit_info := vk.SubmitInfo {
		...
		pWaitDstStageMask    = raw_data(wait_dest_stages),
	...
	vk_check(vk.QueueSubmit(queue, 1, &submit_info, fence), "Failed to submit command buffer!")
}
...
create_descriptor_set_layout :: proc(device: vk.Device) -> vk.DescriptorSetLayout {
	...
	layout_bindings := []vk.DescriptorSetLayoutBinding{ubo_binding, storage_last_frame_binding, storage_current_frame_binding}
...
update_descriptor_set :: proc(
	...
	storage_last_frame: vk.Buffer,
	storage_current_frame: vk.Buffer,
	...
	storage_last_frame_buffer_info := vk.DescriptorBufferInfo {
		buffer = storage_last_frame,
		offset = 0,
		range  = vk.DeviceSize(storage_buffer_size),
	}

	storage_current_frame_buffer_info := vk.DescriptorBufferInfo {
		buffer = storage_current_frame,
		offset = 0,
		range  = vk.DeviceSize(storage_buffer_size),
	}
	...
	storage_last_frame_descriptor_write := vk.WriteDescriptorSet {
		sType           = .WRITE_DESCRIPTOR_SET,
		dstSet          = descriptor_set,
		dstBinding      = 1,
		dstArrayElement = 0,
		descriptorType  = .STORAGE_BUFFER,
		descriptorCount = 1,
		pBufferInfo     = &storage_last_frame_buffer_info,
	}

	storage_current_frame_descriptor_write := vk.WriteDescriptorSet {
		sType           = .WRITE_DESCRIPTOR_SET,
		dstSet          = descriptor_set,
		dstBinding      = 2,
		dstArrayElement = 0,
		descriptorType  = .STORAGE_BUFFER,
		descriptorCount = 1,
		pBufferInfo     = &storage_current_frame_buffer_info,
	}

	descriptor_writes := []vk.WriteDescriptorSet{ubo_descriptor_write, storage_last_frame_descriptor_write, storage_current_frame_descriptor_write}
...
create_compute_pipeline :: proc(
	...
	// Shaders
	// Shader entrypoints
	entry_cstr := strings.clone_to_cstring(entry_point)
	defer delete(entry_cstr)
...
record_compute_command_buffer :: proc(
	command_buffer: vk.CommandBuffer,
	compute_pipeline: vk.Pipeline,
	pipeline_layout: vk.PipelineLayout,
	descriptor_set: vk.DescriptorSet,
...
main :: proc() {
	...
	shader_module := create_shader_module(device, "shader.slang", {"vertMain", "fragMain", "compMain"})
	fmt.println("Shader module... OK")

	// Descriptor set layout
	descriptor_set_layout := create_descriptor_set_layout(device)
	fmt.println("Descriptor set layout... OK")
	...
	// Descriptor sets...
	descriptor_sets := create_descriptor_set(device, descriptor_pool, descriptor_set_layout, NB_FRAMES_IN_FLIGHT)
	fmt.println("Descriptor sets... OK")

	// Create graphics pipeline
	graphics_pipeline, pipeline_layout := create_graphics_pipeline(device, shader_module, "vertMain", "fragMain", swap_chain_format, depth_format, samples)
	fmt.println("Graphics pipeline... OK")

	// Create compute pipeline
	compute_pipeline, compute_pipeline_layout := create_compute_pipeline(device, shader_module, "compMain", descriptor_set_layout)
	fmt.println("Compute pipeline... OK")
	...
	create_command_buffers(device, command_pool, compute_command_buffers[:])
	fmt.println("Compute command buffers... OK")
	...
	// Semaphores for the compute pipeline
	compute_semaphores: [NB_FRAMES_IN_FLIGHT]vk.Semaphore
	create_semaphores(device, compute_semaphores[:])
	fmt.println("Compute semaphores... OK")

	// Fences for the compute pipeline
	...
	delete(particles)
	fmt.println("Particles storage buffers... OK")

	// Set uniform buffer in descriptor sets
	for i in 0 ..< NB_FRAMES_IN_FLIGHT {
		update_descriptor_set(device, descriptor_sets[i], ubo_buffers[i], storage_buffers[(i + 1) % NB_FRAMES_IN_FLIGHT], storage_buffers[i], size_particles_buffer)
	}
	...
	for !glfw.WindowShouldClose(window) && running {
		...
		// Update uniform buffer to the elapsed time
	...
	for compute_fence in compute_fences {
		if compute_fence != 0 {
			vk.DestroyFence(device, compute_fence, nil)
		}
	}
	for compute_semaphore in compute_semaphores {
		if compute_semaphore != 0 {
			vk.DestroySemaphore(device, compute_semaphore, nil)
		}
	}
