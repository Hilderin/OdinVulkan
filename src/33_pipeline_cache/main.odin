...
App :: struct {
	...
	pipeline_cache:           ovk.Pipeline_Cache,
...
init_app :: proc(app: ^App) -> (err: ovk.Error) {
	...
	// Create window
	app.window = ovk.create_window({instance = &app.instance, title = "Pipeline cache: activated", width = 512, height = 512, resizable = true}) or_return
	...
	// Pipeline cache
	cache_path := "bin/pipeline_cache.bin"
	cache_data: []u8
	defer delete(cache_data)

	if os.exists(cache_path) {
		cache_data = os.read_entire_file(cache_path, context.temp_allocator) or_return
	}

	app.pipeline_cache = ovk.create_pipeline_cache({device = &app.device, initial_data = cache_data}) or_return
	...
	cache_data = ovk.get_pipeline_cache_data(&app.pipeline_cache) or_return
	fmt.printfln("Pipeline cache data len: %d", len(cache_data))

	// Write the cache data on disk
	os.write_entire_file_from_bytes(cache_path, cache_data) or_return
...
destroy_app :: proc(app: ^App) {
	...
	ovk.destroy_pipeline_cache(&app.pipeline_cache)
