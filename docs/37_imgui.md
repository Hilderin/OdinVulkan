...
`imgui.odin` is not a pure Odin implementation. It wraps the C API of dear imgui (generated by [dear_bindings](https://github.com/dearimgui/dear_bindings)) and links against a static library. That means vendoring the bindings is not enough: the C library has to be compiled once and placed at `libs/imgui/imgui_windows_x64.lib`, which is where the `foreign import` in the binding looks for it.

The library is built from the dear imgui sources plus the generated C wrapper. The odin-imgui repository ships a [premake5.lua](https://github.com/Capati/odin-imgui/blob/main/premake5.lua) that clones the sources, runs dear_bindings (which needs Python) and generates a `build/build.bat` that compiles the library with MSVC. So building it once requires premake5, Python 3 and Visual Studio build tools. That's a one-time cost: the resulting `.lib` is committed in the repo and the Odin side only needs the binary.
...
### Vendoring and building the binding
...
The C library is built once from that repository:

```c
premake5 --backends=glfw,vulkan vs2022
```

inside `build/` with a `python3` available on the PATH, then:

```c
build.bat
```

The script needs `vcvars64.bat` to be reachable, so run it from a Visual Studio developer prompt or add the `VC\Auxiliary\Build` folder to the PATH. The result is `imgui_windows_x64.lib`, copied to `libs/imgui/imgui_windows_x64.lib`.

The commands above are for Windows only. On Linux and macOS the library has to be built yourself: the [Building section of the odin-imgui README](https://github.com/Capati/odin-imgui#building) explains how, with the same premake5 script (`gmake` on Unix, `xcode4` on macOS). Copy the result - `libimgui_linux_x64.a` on Linux, `libimgui_macosx_x64.a` on macOS - into `libs/imgui`, next to the Windows library.

The Windows library is committed in the repo, so on Windows nobody needs to rebuild it to compile the project. The steps above are only there for completeness, in case the library has to be regenerated from a newer binding.
...
- A link error mentioning `imgui_windows_x64.lib`: the library is missing from `libs/imgui`. Build it once with premake5 + Python + MSVC as described above.
