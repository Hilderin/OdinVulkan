...
init_app :: proc(app: ^App) -> (err: ovk.Error) {
	...
	// Create window
	app.window = ovk.create_window({instance = &app.instance, title = "ImGui", width = 1200, height = 800, resizable = true}) or_return
...
run_app :: proc(app: ^App) -> (err: ovk.Error) {
	...
	for !ovk.window_should_close(&app.window) && app.running {
		...
		// Start an ImGui frame and build the UI. The demo window is a good way
		// to explore the widgets and check the input callbacks work.
		ovk.imgui_new_frame()
		im.ShowDemoWindow()
		im.Render()
