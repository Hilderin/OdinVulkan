...
init_app :: proc(app: ^App) -> (err: ovk.Error) {
	...
	// Create window
	app.window = ovk.create_window({instance = &app.instance, title = "I'm moving the camera by myself", width = 1200, height = 800, resizable = true}) or_return
...
update_uniform_buffer :: proc(ubo_mapped_buffer: ^ovk.Mapped_Buffer, swap_chain_extent: vk.Extent2D, camera: ^Camera) {
...
run_app :: proc(app: ^App) -> (err: ovk.Error) {
	...
	for !ovk.window_should_close(&app.window) && app.running {
		...
		im.Begin("Info")
		im.LabelText("Delta time", "%f", app.delta_time)
		im.LabelText("Camera yaw", "%f", app.camera.yaw)
		im.LabelText("Camera pitch", "%f", app.camera.pitch)
		im.LabelText(
			"Camera forward vec.",
			"%f, %f, %f",
			get_camera_forward_vector(&app.camera).x,
			get_camera_forward_vector(&app.camera).y,
			get_camera_forward_vector(&app.camera).z,
		)
		im.End()
		...
		update_uniform_buffer(&app.ubo_mapped_buffers[app.swap_chain.frame_index], app.swap_chain.extent, &app.camera)
