...
create_image_views :: proc(device: vk.Device, images: []vk.Image, format: vk.Format) -> []vk.ImageView {
	...
	for image, i in images {
		image_views[i] = create_image_view(device, image, format, {.COLOR})
	}
...
transition_image_layout :: proc(
	...
	image_barrier := vk.ImageMemoryBarrier2 {
		...
		image = image,
		subresourceRange = vk.ImageSubresourceRange{aspectMask = image_aspect_flags, baseMipLevel = 0, levelCount = 1, baseArrayLayer = 0, layerCount = 1},
	}
...
begin_rendering :: proc(command_buffer: vk.CommandBuffer, image_view: vk.ImageView, swap_chain_extent: vk.Extent2D, depth_image_view: vk.ImageView) {
...
create_texture_image :: proc(physical_device: vk.PhysicalDevice, device: vk.Device, path: string, command_pool: vk.CommandPool, queue: vk.Queue) -> (vk.Image, vk.DeviceMemory) {

	// Using core image/jpeg to prevent problem with missing stbi on linux systems.
	...
	transition_image_layout(
		command_buffer,
		image,
		.TRANSFER_DST_OPTIMAL, //old_layout
		.SHADER_READ_ONLY_OPTIMAL, //new_layout
		{.TRANSFER_WRITE}, // src_access_mask
		{.SHADER_READ}, // dst_access_mask
		{.TRANSFER}, // src_stage
		{.FRAGMENT_SHADER}, // dst_stage
		{.COLOR}, //image_aspect_flags
	)
	end_single_time_commands(device, command_pool, command_buffer, queue)
...
main :: proc() {
	...
	image, image_memory := create_texture_image(physical_device, device, "../../assets/images/statue.jpg", command_pool, graphics_queue)
