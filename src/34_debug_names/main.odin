...
init_app :: proc(app: ^App) -> (err: ovk.Error) {
	...
	// Create window
	app.window = ovk.create_window({instance = &app.instance, title = "Debug Names", width = 512, height = 512, resizable = true}) or_return
	...
	// Name objects that were created before the device was available
	...
	ovk.set_debug_name(&app.device, &app.swap_chain.swap_chain, "My Swap Chain") or_return
	ovk.set_debug_name(&app.device, &app.swap_chain.depth_image, "My Depth Image") or_return
	ovk.set_debug_name(&app.device, &app.swap_chain.color_image, "My Color Image") or_return
	for &image in app.swap_chain.images {
		ovk.set_debug_name(&app.device, &image, "My Swap Chain Image") or_return
	}

	for i in 0 ..< app.swap_chain.nb_frames_in_flight {
		ovk.set_debug_name(&app.device, &app.swap_chain.draw_fences[i], fmt.tprintf("My Draw Fence {}", i)) or_return
		ovk.set_debug_name(&app.device, &app.swap_chain.acquire_semaphores[i], fmt.tprintf("My Acquire Semaphore {}", i)) or_return
	}
	for i in 0 ..< len(app.swap_chain.images) {
		ovk.set_debug_name(&app.device, &app.swap_chain.submit_semaphores[i], fmt.tprintf("My Submit Semaphore {}", i)) or_return
	}

	// Create shader module
	app.shader = ovk.create_shader({device = &app.device, slang_path = "shader.slang", entry_points = {"vertMain", "fragMain"}}) or_return
	ovk.set_debug_name(&app.device, &app.shader, "My Shader Module") or_return
	...
	ovk.set_debug_name(&app.device, &app.descriptor_set_layout, "My Descriptor Set Layout") or_return
	...
	ovk.set_debug_name(&app.device, &app.descriptor_pool, "My Descriptor Pool") or_return
	...
	for &ds in app.descriptor_sets {
		ovk.set_debug_name(&app.device, &ds, "My Descriptor Set") or_return
	}
	...
	ovk.set_debug_name(&app.device, &app.graphics_pipeline, "My Graphics Pipeline") or_return
	...
	ovk.set_debug_name(&app.device, &app.graphics_command_pool, "My Graphics Command Pool") or_return
	...
	for &cb in app.graphics_command_buffers {
		ovk.set_debug_name(&app.device, &cb, "My Graphics Command Buffer") or_return
	}
	...
	ovk.set_debug_name(&app.device, &app.vertex_buffer, "My Vertex Buffer") or_return
	...
	ovk.set_debug_name(&app.device, &app.index_buffer, "My Index Buffer") or_return
	...
	ovk.set_debug_name(&app.device, &app.texture, "My Texture Image") or_return

	// Sampler
	app.sampler = ovk.create_sampler({device = &app.device}) or_return
	ovk.set_debug_name(&app.device, &app.sampler, "My Texture Sampler") or_return
	...
	for i in 0 ..< app.swap_chain.nb_frames_in_flight {
		ovk.set_debug_name(&app.device, &app.ubo_buffers[i], fmt.tprintf("My UBO Buffer {}", i)) or_return
	}
